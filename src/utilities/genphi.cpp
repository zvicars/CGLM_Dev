...
real computePhiForSingleCellIntegrated(const Vec3<std::size_t>& index, const Vec3<std::size_t>& box_size, real spacing, const AtomFF& atom_in){
  ...
  Vec3<real> size;
  for(int i=0; i<3; i++){
    point[i] = ((real)index[i])*spacing; //place test point in middle of lattice cell
    size[i] = box_size[i]*spacing;
  }
  int counter = 0;
  for(real i = 0.0; i <= 1.0; i+=0.1){
  for(real j = 0.0; j <= 1.0; j+=0.1){
  for(real k = 0.0; k <= 1.0; k+=0.1){
