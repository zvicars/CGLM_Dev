...
template <class T, std::size_t N>
std::array< std::array<T, 1>, N > arr2Col(std::array<T, N> v){
...
template <class T, std::size_t N>
std::array< std::array<T, N>, 1 > arr2Row(std::array<T, N> v){
...
template <class T, std::size_t N>
std::array<T, N> row2Arr(std::array< std::array<T, N>, 1 > v){
  return v[0];
}

template <class T, std::size_t N>
std::array<T, N> col2Arr(std::array< std::array<T, 1>, N > v){
...
std::array< std::array<T, M>, N > matrix_hadamard( std::array< std::array<T, M>, N> left, std::array< std::array<T, M>, N> right){
  std::array< std::array<T, M>, N > output;
  for(int n = 0; n < N; n++){
    for(int m = 0; m < M; m++){
      output[n][m] = left[n][m] * right[n][m]; 
    }
  }
  return output;
}
...
template <class T, std::size_t N, std::size_t M>
std::array< std::array<T, M>, N > matrix_scalar_mult( T left, std::array< std::array<T, M>, N> right){
  std::array< std::array<T, M>, N > output;
  for(int n = 0; n < N; n++){
    for(int m = 0; m < M; m++){
      output[n][m] = left * right[n][m]; 
    }
  }
  return output;
}